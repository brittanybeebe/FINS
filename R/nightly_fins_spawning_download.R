...
log_message <- function(msg, type = "INFO") {
  ...
  # Append to log file
  cat(full_msg, file = "./logs/fins_spawning_download.log", append = TRUE)
}
...
tryCatch({
  ...
  log_message(paste("Requesting ALL historical spawning data through", end_date, "for all of our FINS facilities"))
  # log_message(paste("Requesting ALL historical spawning data through", end_date, "for entire domain"))
  ...
  if (nrow(fins_data) == 0) {
    log_message("API returned empty data set - this may be normal if no recent spawning occurred")
  } else {
  ...
  file_name <- "data/SpawningData.csv"
  ...
  if (nrow(fins_data) > 0) {
    ...
    date_col <- NULL
    if ("Session Date" %in% names(fins_data)) {
      date_col <- "Session Date"
    } else if ("SpawnDate" %in% names(fins_data)) {
      date_col <- "SpawnDate"
    }
